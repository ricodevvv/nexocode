...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "nexocode/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".nexocode/glossary/zh-cn.md"
  if (locale === "zht") return ".nexocode/glossary/zh-tw.md"
  return `.nexocode/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("NexoCode did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("NexoCode returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("NexoCode session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel NexoCode runs for 'all' (default: 4)
      --model <provider/id>  NexoCode model (default: nexocode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running NexoCode
...
`)
  ...
  if (failed.length) console.error(`\nNexoCode failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.NEXOCODE_DISABLE_PROJECT_CONFIG = "1"
  env.NEXOCODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["nexocode", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.NEXOCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["nexocode", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.NEXOCODE_CONFIG
  delete env.NEXOCODE_CONFIG_DIR
  delete env.NEXOCODE_CONFIG_CONTENT
  delete env.NEXOCODE_PERMISSION
  delete env.NEXOCODE_AUTO_SHARE
  return env
}
