...
import { createNexocode } from "@nexocode-ai/sdk"
...
`)
  ...
  const nexocode = await createNexocode({ port: 0 })
  ...
  try {
    ...
    const session = await nexocode.client.session.create()
    const result = await nexocode.client.session
      .prompt({
