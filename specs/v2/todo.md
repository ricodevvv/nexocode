...
The nexocode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@nexocode-ai/llm` tool loop after replacing its
...
- plugins get global "nexocode" instance like in that post i showed
- nexocode instance has stuff like `nexocode.session.prompt()` or
  `nexocode.tool.register({...})`
...
- decide whether to preserve deprecated `@nexocode-ai/llm` orchestration exports
