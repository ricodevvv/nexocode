...
Move the canonical NexoCode terminal application from
`packages/nexocode/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/nexocode ---\
                      > @nexocode-ai/tui -> @nexocode-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/nexocode`,
`packages/cli`, or `@nexocode-ai/core`.

The SDK is the TUI's NexoCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/nexocode`.
- Do not replace private `packages/nexocode` imports with `@nexocode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@nexocode-ai/tui`.

## Ownership Boundary

### `@nexocode-ai/tui` Owns
...
- NexoCode domain data displayed by the TUI
...
```text
packages/nexocode/src/cli/cmd/tui
```

Its private dependency on `packages/nexocode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/nexocode/src/*`.
...
Status: Completed. The private `@nexocode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@nexocode-ai/tui`.
...
- The package has no dependency on `nexocode`, `@nexocode-ai/cli`, or
  `@nexocode-ai/core`.
...
primitives now live in `@nexocode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@nexocode-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads NexoCode globals or process state;
...
Replace process-global NexoCode state with resolved TUI inputs.
...
`@nexocode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/nexocode` initially.
...
canonical in `@nexocode-ai/tui`. Configured references resolve through the new
...
adapters rather than NexoCode domain implementations.

Make the SDK the only NexoCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports NexoCode data and operations only from
  `@nexocode-ai/sdk`.
...
`@nexocode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/nexocode`.
...
- The package has no imports from `packages/nexocode`, `packages/cli`, or
  `@nexocode-ai/core`.
...
public `@nexocode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/nexocode`.
- Keep the legacy embedded worker and server startup in `packages/nexocode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@nexocode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/nexocode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@nexocode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@nexocode-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for NexoCode domain state.
...
cd packages/nexocode && bun typecheck
...
rg '@nexocode-ai/core|packages/nexocode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/nexocode/src packages/nexocode/test
```
...
- Build the current-platform `packages/nexocode` binary.
