# Remove `packages/nexocode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/nexocode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the nexocode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/nexocode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/nexocode/src/account/repo.ts`
- `packages/nexocode/src/cli/cmd/db.ts`
- `packages/nexocode/src/cli/cmd/import.ts`
- `packages/nexocode/src/cli/cmd/stats.ts`
- `packages/nexocode/src/control-plane/workspace.ts`
- `packages/nexocode/src/index.ts`
- `packages/nexocode/src/node.ts`
- `packages/nexocode/src/permission/index.ts`
- `packages/nexocode/src/project/project.ts`
- `packages/nexocode/src/server/projectors.ts`
- `packages/nexocode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/nexocode/src/server/shared/fence.ts`
- `packages/nexocode/src/session/message-v2.ts`
- `packages/nexocode/src/session/projectors.ts`
- `packages/nexocode/src/session/prompt.ts`
- `packages/nexocode/src/session/session.ts`
- `packages/nexocode/src/session/todo.ts`
- `packages/nexocode/src/share/share-next.ts`
- `packages/nexocode/src/storage/db.ts`
- `packages/nexocode/src/sync/index.ts`
- `packages/nexocode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy nexocode database wrapper; `packages/nexocode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the nexocode wrapper.
...
Status: Completed. `SyncEvent` and the nexocode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy nexocode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy nexocode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy nexocode database wrapper.
...
All migration groups are complete or superseded. `packages/nexocode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No nexocode data-migration group remains.

The previous nexocode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate nexocode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/nexocode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/nexocode/src`
- `bun typecheck` from `packages/nexocode`
- Relevant package tests from `packages/nexocode`, not the repo root
