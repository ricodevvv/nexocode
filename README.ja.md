...
# パッケージマネージャー
npm i -g nexocode-ai@latest        # bun/pnpm/yarn でもOK
scoop install nexocode             # Windows
choco install nexocode             # Windows
brew install ricodevvv/tap/nexocode # macOS と Linux（推奨。常に最新）
brew install nexocode              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S nexocode            # Arch Linux (Stable)
paru -S nexocode-bin               # Arch Linux (Latest from AUR)
mise use -g nexocode               # どのOSでも
nix run nixpkgs#nexocode           # または github:ricodevvv/nexocode で最新 dev ブランチ
```
...
NexoCode はデスクトップアプリとしても利用できます。[releases page](https://github.com/ricodevvv/nexocode/releases) から直接ダウンロードするか、[nexocode.ai/download](https://nexocode.ai/download) を利用してください。
...
1. `$NEXOCODE_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.nexocode/bin` - デフォルトのフォールバック
...
NexoCode には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://nexocode.ai/docs/agents) の詳細はこちら。

### ドキュメント

NexoCode の設定については [**ドキュメント**](https://nexocode.ai/docs) を参照してください。

### コントリビュート

NexoCode に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### NexoCode の上に構築する

NexoCode に関連するプロジェクトで、名前に "nexocode"（例: "nexocode-dashboard" や "nexocode-mobile"）を含める場合は、そのプロジェクトが NexoCode チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/nexocode) | [X.com](https://x.com/nexocode)
