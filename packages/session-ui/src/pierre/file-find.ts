...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("nexocode-find")
  api.delete("nexocode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("nexocode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("nexocode-find", new Highlight(...rest))
    return true
  }
