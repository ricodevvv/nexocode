...
import { createNexocode, type ToolPart } from "@nexocode-ai/sdk"
...
console.log("🚀 Starting nexocode server...")
const nexocode = await createNexocode({
  port: 0,
})
console.log("✅ Nexocode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await nexocode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new nexocode session...")
    const { client, server } = nexocode
    ...
    console.log("✅ Created nexocode session:", createResult.data.id)
  ...
  console.log("📝 Sending to nexocode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Nexocode response:", JSON.stringify(result, null, 2))
