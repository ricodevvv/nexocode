...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-nexocode" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslNexocodeAction(check?: WslNexocodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installNexocode"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateNexocode"
}
...
export function addServerViewModel(input: {
  ...
  const nexocodeCheck = selectedDistro ? (state?.nexocodeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.nexocodeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateNexocode" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.nexocodeMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installNexocode" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  nexocodeCheck: WslNexocodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingNexocode =
    input.state?.job?.kind === "install-nexocode" && input.state.job.distro === input.selectedDistro
  if (!ready || probingNexocode) {
    return {
      variant: "contrast",
  ...
  if (!addServerNexocodeReady(input.nexocodeCheck)) {
    const update = !!input.nexocodeCheck?.resolvedPath && input.nexocodeCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingNexocode
        ? { key: "wsl.onboarding.updatingNexocode" }
        : update
          ? { key: "wsl.onboarding.updateNexocode" }
          : { key: "wsl.onboarding.installNexocode" },
      disabled: !!input.state?.job || input.adding,
      action: "install-nexocode",
      loading: installingNexocode,
...
function addServerNexocodeReady(check: WslNexocodeCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.nexocodeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.nexocodeChecks[item.name]) return [`nexocode:${item.name}`]
    return []
  })
