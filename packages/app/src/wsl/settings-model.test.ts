...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when NexoCode needs attention", () => {
    expect(wslNexocodeAction(undefined)).toBeUndefined()
    expect(
      wslNexocodeAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installNexocode")
    expect(
      wslNexocodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/nexocode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updateNexocode")
    expect(
      wslNexocodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/nexocode",
  ...
  test("does not report ready when NexoCode is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        nexocodeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.nexocode/bin/nexocode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "nexocode is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installNexocode" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-nexocode")
  })
