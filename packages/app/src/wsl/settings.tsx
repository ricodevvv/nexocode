...
import { wslNexocodeAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.nexocodeChecks[item.config.distro]
          const nexocodeAction = () => wslNexocodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-nexocode" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={nexocodeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installNexocode(item.config.distro))}
                    >
