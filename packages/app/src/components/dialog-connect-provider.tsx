import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@nexocode-ai/client/promise"
import { Button } from "@nexocode-ai/ui/button"
import { useDialog } from "@nexocode-ai/ui/context/dialog"
import { Dialog } from "@nexocode-ai/ui/dialog"
import { Icon } from "@nexocode-ai/ui/icon"
import { IconButton } from "@nexocode-ai/ui/icon-button"
import { List, type ListRef } from "@nexocode-ai/ui/list"
import { ProviderIcon } from "@nexocode-ai/ui/provider-icon"
import { Spinner } from "@nexocode-ai/ui/spinner"
import { Tag } from "@nexocode-ai/ui/tag"
import { TextField } from "@nexocode-ai/ui/text-field"
import { ButtonV2 } from "@nexocode-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@nexocode-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@nexocode-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "nexocode-go") return language.t("dialog.provider.nexocodeGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "nexocode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.nexocode.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["nexocode", "nexocode-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "nexocode" || provider.id === "nexocode-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "nexocode"
                                ? "dialog.provider.nexocode.tagline"
                                : "dialog.provider.nexocodeGo.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "nexocode" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "nexocode-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "nexocode"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.nexocodeZen.line1")}</div>
              <div>{language.t("provider.connect.nexocodeZen.line2")}</div>
              <div>
                {language.t("provider.connect.nexocodeZen.visit.prefix")}
                <ExternalLink
                  href="https://nexocode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.nexocodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.nexocodeZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "nexocode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.nexocodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.nexocodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.nexocodeZen.visit.prefix")}
                <ExternalLink href="https://nexocode.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.nexocodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.nexocodeZen.visit.suffix")}
              </div>
