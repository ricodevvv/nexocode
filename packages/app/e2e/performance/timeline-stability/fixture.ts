...
import { SessionStatusEvent } from "@nexocode-ai/schema/session-status-event"
import { SessionV1 } from "@nexocode-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/NexoCode/TimelineStability"
...
export const model = { providerID: "nexocode", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("nexocode.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
