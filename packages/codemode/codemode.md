...
This is the living design and status document for `@nexocode-ai/codemode` and its existing V2 NexoCode adapter.
...
`packages/nexocode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
