...
import { Watcher } from "@nexocode-ai/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    NEXOCODE_EXPERIMENTAL_FILEWATCHER: "true",
    NEXOCODE_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@nexocode.test`.cwd(tmp.path).quiet()
