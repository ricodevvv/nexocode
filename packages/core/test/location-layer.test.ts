...
import { define } from "@nexocode-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@nexocode-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "nexocode.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "nexocode.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
