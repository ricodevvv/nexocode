...
import { ModelsDevPlugin } from "@nexocode-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.NEXOCODE_MODELS_PATH,
          disabled: Flag.NEXOCODE_DISABLE_MODELS_FETCH,
        }
        Flag.NEXOCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.NEXOCODE_MODELS_PATH = previous.path
          Flag.NEXOCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
