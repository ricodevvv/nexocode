...
import type { IntegrationOAuthMethodRegistration } from "@nexocode-ai/plugin/v2/effect/integration"
import { define } from "@nexocode-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://nexocode.ai/console"
const clientID = "nexocode-cli"
...
export const NexocodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "nexocode",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("NexocodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("nexocode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load NexoCode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("nexocode", (integration) => {
        integration.name = "NexoCode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "nexocode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("nexocode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("nexocode")
      ...
      const item = catalog.provider.get(ProviderV2.ID.nexocode)
      if (!item) return
      const hasKey = Boolean(process.env.NEXOCODE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("nexocode")),
