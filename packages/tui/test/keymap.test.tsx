...
import { getNexocodeModeStack, NEXOCODE_BASE_MODE, NexocodeKeymapProvider, registerNexocodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerNexocodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when nexocode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getNexocodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getNexocodeModeStack(keymap).push("autocomplete")
