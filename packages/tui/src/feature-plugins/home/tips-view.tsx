...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("nexocode.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public nexocode.ai link",
  ...
  "Create {highlight}nexocode.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/nexocode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.nexocode/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.nexocode/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.nexocode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.nexocode/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent NexoCode from reading sensitive files",
  "Use {highlight}nexocode run{/highlight} for non-interactive scripting",
  "Use {highlight}nexocode --continue{/highlight} to resume the last session",
  "Use {highlight}nexocode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}nexocode serve{/highlight} for headless API access to NexoCode",
  "Use {highlight}nexocode run --attach{/highlight} to connect to a running server",
  "Run {highlight}nexocode upgrade{/highlight} to update to the latest version",
  "Run {highlight}nexocode auth list{/highlight} to see all configured providers",
  "Run {highlight}nexocode agent create{/highlight} for guided agent creation",
  "Use {highlight}/nexocode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}nexocode github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/nexocode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.nexocode/themes/{/highlight} directory",
  ...
  "Run {highlight}nexocode debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/ricodevvv/nexocode{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with NexoCode Zen for curated, tested models",
