import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@nexocode-ai/plugin/tui"
...
import type { useNexocodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useNexocodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getNexocodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getNexocodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
