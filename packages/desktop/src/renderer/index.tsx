...
import "./styles.css"
import { Splash } from "@nexocode-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.NEXOCODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "nexocode:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__NEXOCODE__ ??= {}
  const pending = window.__NEXOCODE__.deepLinks ?? []
  window.__NEXOCODE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `nexocode.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://nexocode.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("nexocode.global.dat").getItem("language")
