import type { WslDistroProbe, WslNexocodeCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  nexocodeChecks: Record<string, WslNexocodeCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextNexocodeChecks = { ...nexocodeChecks }
  delete nextDistroProbes[distro]
  delete nextNexocodeChecks[distro]
  return { distroProbes: nextDistroProbes, nexocodeChecks: nextNexocodeChecks }
}
