...
let releaseNexocodeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectNexocodeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectNexocodeVersion("1.14.35", "1.16.2")).toThrow(
    "NexoCode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating NexoCode", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.nexocode/bin/nexocode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, nexocodeChecks: {} })
})
...
test("ignores stale background NexoCode checks after removing a WSL server", async () => {
  persistedServers = []
  releaseNexocodeResolve = undefined
  ...
  await waitFor(() => !!releaseNexocodeResolve)
  await controller.removeServer("wsl:Debian")
  releaseNexocodeResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().nexocodeChecks).toEqual({})
})

test("ignores stale startup NexoCode checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking NexoCode", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const nexocode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveNexocode: async (distro) => {
      nexocode.push(distro)
      return "/home/me/.nexocode/bin/nexocode"
    },
  })
  ...
  expect(nexocode).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(nexocode).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().nexocodeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check NexoCode in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const nexocode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(nexocode).toEqual(["Debian"])
  expect(Object.keys(controller.getState().nexocodeChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveNexocode: async () => {
      await new Promise<void>((resolve) => {
        releaseNexocodeResolve = resolve
      })
      return "/home/me/.nexocode/bin/nexocode"
    },
  }
