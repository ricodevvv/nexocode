...
import { expectNexocodeVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveNexocode?: typeof resolveWslNexocode
...
export function createWslServersController(
  ...
  const setNexocodeCheck = (distro: string, check: WslNexocodeCheck) => {
    setState({
      nexocodeChecks: {
        ...state.nexocodeChecks,
        [distro]: check,
      },
    })
  }

  const checkNexocode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveNexocode ?? resolveWslNexocode)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return nexocodeCheck(distro, resolved, version, appVersion)
  }

  const refreshNexocodeCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setNexocodeCheck(distro, await checkNexocode(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const nexocodeChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.nexocodeChecks[distro])
        .map(async (distro) => [distro, await checkNexocode(distro, opts)] as const),
    )
    if (nexocodeChecks.length) {
      setState({ nexocodeChecks: { ...state.nexocodeChecks, ...Object.fromEntries(nexocodeChecks) } })
    }
  }
  ...
  const refreshNexocodeCheckBackground = (id: string, distro: string) => {
    void checkNexocode(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setNexocodeCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl nexocode check failed", { id, distro, message })
      })
  }

  const refreshNexocodeChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkNexocode(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setNexocodeCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl nexocode check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshNexocodeCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installNexocode(name: string) {
      await runJob({ kind: "install-nexocode", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslNexocode(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || nativeT("desktop.wsl.error.installNexocode"))
        }
        await refreshNexocodeCheck(name, { signal: abort.signal })
        expectNexocodeVersion(state.nexocodeChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.nexocodeChecks, distro) : {}),
      })
    },
...
function nexocodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.nexocodeMissing"),
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.nexocodeCannotRun"),
    }
  }
  return {
    distro,
