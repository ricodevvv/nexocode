...
import { type WslCommandLine, resolveWslNexocode, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const nexocode = await resolveWslNexocode(distro)
  if (!nexocode) throw new Error(nativeT("desktop.wsl.error.nexocodeNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "nexocode"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export NEXOCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export NEXOCODE_CLIENT=desktop",
    `export NEXOCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export NEXOCODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(nexocode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
