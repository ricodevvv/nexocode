...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier NexoCode launch", () => {
    expect(hasExistingAppState([file("nexocode.settings")])).toBe(true)
    expect(hasExistingAppState([file("nexocode.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("nexocode")])).toBe(true)
  })
})
