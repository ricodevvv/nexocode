...
export async function checkPluginCompatibility(target: string, nexocodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(nexocodeVersion) || semver.major(nexocodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.nexocode
  if (typeof range !== "string") return
  if (!semver.satisfies(nexocodeVersion, range)) {
    throw new Error(`Plugin requires nexocode ${range} but running ${nexocodeVersion}`)
  }
}
