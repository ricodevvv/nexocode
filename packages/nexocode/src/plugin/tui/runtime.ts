...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@nexocode-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@nexocode-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@nexocode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@nexocode-ai/tui/context/theme"
import { Global } from "@nexocode-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@nexocode-ai/core/util/flock"
import { Flag } from "@nexocode-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@nexocode-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@nexocode-ai/core/v1/config/plugin"
import { createCommandShim } from "@nexocode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@nexocode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".nexocode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".nexocode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".nexocode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.NEXOCODE_PURE ? [] : pluginOrigins
    if (Flag.NEXOCODE_PURE && pluginOrigins.length) {
    }
