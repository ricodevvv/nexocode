...
import type { AssistantMessage, Message, NexocodeClient, Session, SessionMessageResponse } from "@nexocode-ai/sdk/v2"
...
export const AuthMethodID = "nexocode-login"
...
export class Service extends Context.Service<Service, Interface>()("@nexocode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `nexocode auth login` in the terminal",
      name: "Login with nexocode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: NexocodeClient) {
...
function makeUsageService(sdk: NexocodeClient) {
...
async function loadDirectorySnapshot(sdk: NexocodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, nexocode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const nexocodeProvider = providers[ProviderV2.ID.make("nexocode")]
  const nexocodeModel = nexocodeProvider ? Provider.sort(Object.values(nexocodeProvider.models))[0] : undefined
  if (nexocodeProvider && nexocodeModel) return { providerID: nexocodeProvider.id, modelID: nexocodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "NexoCode service failure", service })
}
