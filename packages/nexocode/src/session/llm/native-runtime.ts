...
import type { LLMClientShape } from "@nexocode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("nexocode"))
    return { type: "unsupported", reason: "provider is not openai, nexocode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @nexocode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains nexocode-owned. The native runtime only adapts
      // the @nexocode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
