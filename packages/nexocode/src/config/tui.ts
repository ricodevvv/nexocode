...
import { isRecord } from "@nexocode-ai/tui/util/record"
...
import { TuiKeybind } from "@nexocode-ai/tui/config/keybind"
...
import { TuiConfig } from "@nexocode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@nexocode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old nexocode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.nexocode`
  // folders between cwd and home, and NEXOCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.NEXOCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit NEXOCODE_TUI_CONFIG override, if set.
  if (Flag.NEXOCODE_TUI_CONFIG) {
    const configFile = Flag.NEXOCODE_TUI_CONFIG
  ...
  // 4. `.nexocode` directories (and NEXOCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".nexocode") || dir === Flag.NEXOCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".nexocode") && dir !== Flag.NEXOCODE_CONFIG_DIR) continue
