...
const TUI_SCHEMA_URL = "https://nexocode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from nexocode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const nexocode = await nexocodeFiles(input)
  for (const file of nexocode) {
...
async function nexocodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "nexocode"),
    ...(await Filesystem.findUp(["nexocode.json", "nexocode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "nexocode"))
  }
  if (Flag.NEXOCODE_CONFIG) files.push(Flag.NEXOCODE_CONFIG)
