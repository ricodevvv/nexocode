...
import { serviceUse } from "@nexocode-ai/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@nexocode-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@nexocode-ai/core/v1/config/console-state"
import { FSUtil } from "@nexocode-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@nexocode-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@nexocode-ai/core/v1/config/config"
import { RemoteAuthError } from "@nexocode-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@nexocode-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@nexocode/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["nexocode.jsonc", "nexocode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://nexocode.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.NEXOCODE_CONFIG && !Flag.NEXOCODE_CONFIG_DIR && !Flag.NEXOCODE_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://nexocode.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "nexocode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "nexocode.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://nexocode.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "NEXOCODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/nexocode`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://nexocode.ai/config.json"
        ...
        if (Flag.NEXOCODE_CONFIG) {
          yield* merge(Flag.NEXOCODE_CONFIG, yield* loadFile(Flag.NEXOCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.NEXOCODE_CONFIG })
        }

        if (!Flag.NEXOCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("nexocode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.NEXOCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from NEXOCODE_CONFIG_DIR", { path: Flag.NEXOCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".nexocode") || dir === Flag.NEXOCODE_CONFIG_DIR) {
            for (const file of ["nexocode.json", "nexocode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.nexocode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.NEXOCODE_CONFIG_CONTENT) {
          const source = "NEXOCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.NEXOCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from NEXOCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["NEXOCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("NEXOCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.NEXOCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.NEXOCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("NEXOCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.NEXOCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.NEXOCODE_DISABLE_PRUNE) {
