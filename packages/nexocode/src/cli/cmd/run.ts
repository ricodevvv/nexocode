import type { PermissionV1 } from "@nexocode-ai/core/v1/permission"
import { FSUtil } from "@nexocode-ai/core/fs-util"
// CLI entry point for `nexocode run` and `nexocode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`nexocode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`nexocode --mini --attach`): connects to a running
//      nexocode server and runs interactive mode against it.
//
...
import { createNexocodeClient, type NexocodeClient, type ToolPart } from "@nexocode-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<NexocodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run nexocode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running nexocode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to NEXOCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to NEXOCODE_SERVER_USERNAME or 'nexocode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: NexocodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: NexocodeClient, sessionID: string) {
      ...
      async function current(sdk: NexocodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: NexocodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: NexocodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: NexocodeClient) {
        ...
        async function loop(client: NexocodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createNexocodeClient({
        baseUrl: "http://nexocode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
