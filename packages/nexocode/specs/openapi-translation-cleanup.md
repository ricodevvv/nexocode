...
Trim `packages/nexocode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/nexocode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/nexocode`.
- `[x]` `bun typecheck` from `packages/nexocode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/nexocode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/nexocode`.
- `[x]` `bun dev generate > /tmp/nexocode-openapi.json` from `packages/nexocode`.
...
- Edit `packages/nexocode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/nexocode-openapi.json` from `packages/nexocode`.
...
- Check source schemas in files like `packages/nexocode/src/session/schema.ts`, `packages/nexocode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/nexocode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/nexocode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
