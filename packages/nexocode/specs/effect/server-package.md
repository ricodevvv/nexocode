...
Practical reference for a future `packages/server` split after the nexocode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/nexocode`.
...
Until enough shared service code lives outside `packages/nexocode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/nexocode`

It should not import `packages/nexocode` services while `packages/nexocode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on nexocode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/nexocode` implementation details.
