...
Delete `packages/nexocode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/nexocode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/nexocode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/nexocode`
- TUI plugin loader tests in `packages/nexocode` if runtime plugin API wiring changed
