...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "nexocode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "nexocode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nexocode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nexocode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["nexocode.json", "nexocode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "nexocode.json"),
        path.join(project, "nexocode.jsonc"),
      ])
    })
  })
