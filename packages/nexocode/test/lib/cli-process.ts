// Subprocess test harness for the nexocode CLI. Spawns the real binary against
...
//
// Configuration flows through nexocode's built-in test affordances:
//   - NEXOCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - NEXOCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - NEXOCODE_DISABLE_PROJECT_CONFIG : skip walking up for nexocode.json
//   - NEXOCODE_PURE                : skip external plugin discovery + install
//   - NEXOCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `nexocode.run` is fully wired. The shape supports adding more
// builders (`nexocode.serve(opts)`, `nexocode.acp(opts)`, `nexocode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of NexocodeCli.
...
import { AppProcess } from "@nexocode-ai/core/process"
...
const nexocodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(nexocodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    NEXOCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    NEXOCODE_CONFIG_CONTENT: configJson,
    NEXOCODE_DISABLE_PROJECT_CONFIG: "1",
    NEXOCODE_PURE: "1",
    NEXOCODE_DISABLE_AUTOUPDATE: "1",
    NEXOCODE_DISABLE_AUTOCOMPACT: "1",
    NEXOCODE_DISABLE_MODELS_FETCH: "1",
    NEXOCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `nexocode run`. New flags should
...
// `nexocode serve` is a long-lived process — it never exits on its own.
...
// `nexocode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type NexocodeCli = {
  ...
  // Spawn `nexocode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `nexocode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("nexocode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          NEXOCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("nexocode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("nexocode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "nexocode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `nexocode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("nexocode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const nexocode: NexocodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, nexocode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `nexocode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "nexocode") {
...
// every call site. Use this for any test that needs the nexocode CLI fixture.
//
...
// resources (e.g. `nexocode.serve`) without an extra `Effect.scoped` wrapper —
