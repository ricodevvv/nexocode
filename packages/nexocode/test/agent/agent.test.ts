...
it.instance("plan agent denies edits except .nexocode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".nexocode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".nexocode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.NEXOCODE_TEST_HOME
      process.env.NEXOCODE_TEST_HOME = test.directory
