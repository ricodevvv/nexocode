...
import { EventSequenceTable } from "@nexocode-ai/core/event/sql"
...
import { WorkspaceV2 } from "@nexocode-ai/core/workspace"
import { WorkspaceTable } from "@nexocode-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  NEXOCODE_AUTH_CONTENT: process.env.NEXOCODE_AUTH_CONTENT,
  NEXOCODE_EXPERIMENTAL_WORKSPACES: process.env.NEXOCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.NEXOCODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@nexocode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.NEXOCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=nexocode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.NEXOCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.NEXOCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.NEXOCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=nexocode-test")
