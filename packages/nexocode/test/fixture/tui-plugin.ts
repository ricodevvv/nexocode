...
import type { HostPluginApi } from "@nexocode-ai/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createNexocodeClient({
  ...
  let selected = opts.theme?.selected ?? "nexocode"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "nexocode.default",
