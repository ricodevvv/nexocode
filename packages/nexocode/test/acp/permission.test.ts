...
import type { Event, NexocodeClient } from "@nexocode-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<NexocodeClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "nexocode-acp-permission-"))
