...
import { LLMClient, RequestExecutor } from "@nexocode-ai/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const nexocodeFixture = { providerID: "nexocode-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for nexocode providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(nexocodeFixture.providerID),
          ModelV2.ID.make(nexocodeFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(nexocodeFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [nexocodeFixture.providerID],
          provider: {
            [nexocodeFixture.providerID]: {
              name: "NexoCode Test",
