...
import { SessionV1 } from "@nexocode-ai/core/v1/session"
import { ModelsDev } from "@nexocode-ai/core/models-dev"
import { HttpRecorder } from "@nexocode-ai/http-recorder"
import { HttpRecorderInternal } from "@nexocode-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@nexocode-ai/llm"
import { RequestExecutor } from "@nexocode-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.nexocode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.NEXOCODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["nexocode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NEXOCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("NEXOCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["nexocode", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "nexocode-proxy",
    name: "NexoCode proxy",
    providerID: ProviderV2.ID.nexocode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["nexocode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.NEXOCODE_RECORD_CONSOLE_TOKEN && process.env.NEXOCODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.nexocode,
        name: "NexoCode Zen",
        env: ["NEXOCODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.NEXOCODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.NEXOCODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.NEXOCODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["nexocode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NEXOCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("NEXOCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("NEXOCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set NEXOCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the nexocode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "nexocode.json"),
      JSON.stringify({ $schema: "https://nexocode.ai/config.json", ...scenario.config(model) }),
    ),
  )
