...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-nexocode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-nexocode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://nexocode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-nexocode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-nexocode-workspace")
