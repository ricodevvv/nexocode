...
describe("nexocode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // NEXOCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ nexocode }) =>
      Effect.gen(function* () {
        const r = yield* nexocode.spawn(["mcp", "list"])
        nexocode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nexocode }) =>
      Effect.gen(function* () {
        const r = yield* nexocode.spawn(["providers", "list"])
        nexocode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ nexocode }) =>
      Effect.gen(function* () {
        const r = yield* nexocode.spawn(["models"])
        nexocode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ nexocode }) =>
      Effect.gen(function* () {
        const r = yield* nexocode.spawn(["agent", "list"])
        nexocode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh NEXOCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ nexocode }) =>
      Effect.gen(function* () {
        const r = yield* nexocode.spawn(["session", "list"])
        nexocode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nexocode }) =>
      Effect.gen(function* () {
        const r = yield* nexocode.spawn(["stats"])
        nexocode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under NEXOCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ nexocode }) =>
      Effect.gen(function* () {
        const r = yield* nexocode.spawn(["db", "path"])
        nexocode.expectExit(r, 0, "db path")
