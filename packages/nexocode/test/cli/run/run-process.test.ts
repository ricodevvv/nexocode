// Subprocess integration tests for `nexocode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `nexocode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `NEXOCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("nexocode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexocode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexocode.run("say hi")
        nexocode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexocode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexocode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexocode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* nexocode.run("think", { extraArgs: ["--thinking"] })
        nexocode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* nexocode.run("think again")
        nexocode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexocode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexocode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexocode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexocode.run("say hi", { format: "json" })
        nexocode.expectExit(result, 0)

        const events = nexocode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ nexocode }) =>
      Effect.gen(function* () {
        const result = yield* nexocode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexocode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexocode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexocode }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* nexocode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, nexocode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* nexocode.run("request permission", { permission: { bash: "ask" } })
        nexocode.expectExit(denied, 0)
        ...
        const allowed = yield* nexocode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        nexocode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* nexocode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        nexocode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, nexocode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* nexocode.serve()

        const result = yield* nexocode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, nexocode }) =>
      Effect.gen(function* () {
        const result = yield* nexocode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, nexocode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* nexocode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
