...
import { NexocodeClient, type GlobalEvent } from "@nexocode-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<NexocodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<NexocodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<NexocodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<NexocodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<NexocodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://nexocode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: NexocodeClient["event"]["subscribe"]
    globalEvent?: NexocodeClient["global"]["event"]
    promptAsync?: NexocodeClient["session"]["promptAsync"]
    status?: NexocodeClient["session"]["status"]
    messages?: NexocodeClient["session"]["messages"]
    children?: NexocodeClient["session"]["children"]
    permissions?: NexocodeClient["permission"]["list"]
    questions?: NexocodeClient["question"]["list"]
  } = {},
) {
  const client = new NexocodeClient()

  const subscribe: NexocodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: NexocodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: NexocodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: NexocodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: NexocodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: NexocodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: NexocodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: NexocodeClient["question"]["list"] = input.questions ?? (() => ok([]))
