...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ nexocode }) =>
    Effect.gen(function* () {
      const result = yield* nexocode.spawn(["--replay-limit", "10"])

      nexocode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ nexocode }) =>
    Effect.gen(function* () {
      const result = yield* nexocode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ nexocode }) =>
    Effect.gen(function* () {
      const result = yield* nexocode.spawn(["--mini", "--port", "4096"])
