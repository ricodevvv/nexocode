...
describe("debug config redaction", () => {
  cliIt.live("always masks resolved credentials", ({ nexocode }) =>
    Effect.gen(function* () {
      const content = JSON.stringify({ provider: config.provider })
      const env = { NEXOCODE_CONFIG_CONTENT: content }
      const result = yield* nexocode.spawn(["debug", "config"], { env })
      nexocode.expectExit(result, 0, "debug config")
