...
import { and, Database, eq, isNull, lt, or, sql } from "@nexocode-ai/console-core/drizzle/index.js"
import { KeyTable } from "@nexocode-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@nexocode-ai/console-core/schema/billing.sql.js"
...
import { BlackData } from "@nexocode-ai/console-core/black.js"
import { UserTable } from "@nexocode-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@nexocode-ai/console-core/schema/model.sql.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-nexocode-session") ?? ""
    const requestId = input.request.headers.get("x-nexocode-request") ?? ""
    const ocClient = input.request.headers.get("x-nexocode-client") ?? ""
    const projectId = input.request.headers.get("x-nexocode-project") ?? ""
    ...
    if (authInfo && opts.modelList === "lite" && requiresGoTrainingConsent(modelInfo.id) && !authInfo.allowTraining)
      throw new DataPolicyError(
        t("zen.api.error.trainingNotAllowed", {
          consoleGoUrl: `https://nexocode.ai/workspace/${authInfo.workspaceID}/go`,
        }),
      )
    ...
    const providerRequest = async () => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          if (!isNewInference) {
            headers.delete("x-nexocode-session")
            headers.delete("x-nexocode-project")
            headers.delete("x-nexocode-client")
            headers.delete("x-nexocode-request")
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-nexocode-endpoint-id")
        const resEndpointModelId = res.headers.get("x-nexocode-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      ...
      try {
        const consoleGoUrl = `https://nexocode.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://nexocode.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://nexocode.ai/workspace/${authInfo.workspaceID}/members`
