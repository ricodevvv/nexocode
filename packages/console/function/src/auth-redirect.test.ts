...
describe("authorization redirect validation", () => {
  test("allows registered NexoCode callbacks", () => {
    expect(isAllowedAuthorizationRedirect("app", "https://nexocode.ai/auth/callback")).toBe(true)
    expect(isAllowedAuthorizationRedirect("app", "https://dev.nexocode.ai/auth/callback")).toBe(true)
  ...
  test("rejects unregistered clients and external redirects", () => {
    expect(isAllowedAuthorizationRedirect("other", "https://nexocode.ai/auth/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://evil.example/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://nexocode.ai.evil.example/callback")).toBe(false)
