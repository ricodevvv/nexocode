...
import { createNexocodeClient } from "@nexocode-ai/sdk/v2/client"
import { ServerAuth } from "@nexocode-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createNexocodeClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@nexocode/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createNexocodeClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createNexocodeClient({ baseUrl: connection.url, headers: connection.headers })
    })
