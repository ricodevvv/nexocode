...
description: Usa la configurazione JSON di NexoCode.
---

Puoi configurare NexoCode usando un file di configurazione JSON.

---

## Formato

NexoCode supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/nexocode`) - default dell'organizzazione
2. **Config globale** (`~/.config/nexocode/nexocode.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `NEXOCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`nexocode.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.nexocode`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `NEXOCODE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.nexocode` e `~/.config/nexocode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/nexocode`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di NexoCode in `~/.config/nexocode/nexocode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/nexocode/tui.json`.
...
Aggiungi `nexocode.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando NexoCode si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `NEXOCODE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `NEXOCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.nexocode` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.nexocode`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`nexocode.ai/config.json`**](https://nexocode.ai/config.json).

La configurazione TUI usa [**`nexocode.ai/tui.json`**](https://nexocode.ai/tui.json).
...
Usa `NEXOCODE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `nexocode.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `nexocode serve` e `nexocode web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server NexoCode.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `nexocode.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in NexoCode tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, NexoCode prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/nexocode/agents/` o `.nexocode/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, NexoCode fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`nexocode run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/nexocode/commands/` o `.nexocode/commands/`. [Scopri di piu](/docs/commands).
...
NexoCode scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, nexocode **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono NexoCode con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.nexocode/plugins/` o `~/.config/nexocode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare NexoCode a usare solo alcuni provider invece di disabilitarli uno per uno.
