---
title: Plugin
description: Scrivi plugin per estendere NexoCode.
---

I plugin ti permettono di estendere NexoCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di NexoCode.
...
- `.nexocode/plugins/` - plugin a livello progetto
- `~/.config/nexocode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/nexocode/node_modules/`.
...
1. Config globale (`~/.config/nexocode/nexocode.json`)
2. Config di progetto (`nexocode.json`)
3. Directory plugin globale (`~/.config/nexocode/plugins/`)
4. Directory plugin di progetto (`.nexocode/plugins/`)
...
NexoCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di nexocode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere nexocode.
...
:::note
Se usi l'app desktop di NexoCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a nexocode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a nexocode:
...
L'helper `tool` crea uno strumento personalizzato che nexocode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in nexocode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://nexocode.ai/docs/sdk) per i dettagli.
