---
title: Server
description: Interagisci con il server nexocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `nexocode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client nexocode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `nexocode.local` |
...
Imposta `NEXOCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `nexocode`, oppure imposta `NEXOCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `nexocode serve` sia per `nexocode web`.
...
Quando esegui `nexocode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server nexocode per interagire con nexocode in modo programmatico.
:::

Questa architettura permette a nexocode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `nexocode serve` per avviare un server standalone. Se la TUI di nexocode e gia in esecuzione, `nexocode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di NexoCode.
...
Il server nexocode espone le seguenti API.
