...
Le skill degli agenti permettono a NexoCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
NexoCode cerca in queste posizioni:

- Config di progetto: `.nexocode/skills/<name>/SKILL.md`
- Config globale: `~/.config/nexocode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, NexoCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.nexocode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/nexocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.nexocode/skills/git-release/SKILL.md` cosi':
...
NexoCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `nexocode.json`:
...
**Per agenti integrati** (in `nexocode.json`):
