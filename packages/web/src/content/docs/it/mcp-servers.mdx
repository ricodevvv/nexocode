...
Puoi aggiungere strumenti esterni a NexoCode usando il _Model Context Protocol_, o MCP. NexoCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [NexoCode Config](https://nexocode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/nexocode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
NexoCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, NexoCode:
...
Se il server richiede autenticazione, NexoCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `nexocode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, NexoCode memorizzera' i token in modo sicuro in `~/.local/share/nexocode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in NexoCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione NexoCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare NexoCode al tuo account Sentry.
