...
مع NexoCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم NexoCode سمتنا `nexocode`.
...
يأتي NexoCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق NexoCode مظهر terminal لديهم
...
يدعم NexoCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/nexocode/themes/*.json` أو `$XDG_CONFIG_HOME/nexocode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.nexocode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.nexocode/themes/*.json`
