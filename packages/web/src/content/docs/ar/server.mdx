---
title: الخادم
description: تفاعل مع خادم nexocode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `nexocode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل nexocode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `nexocode.local` |
...
عيّن `NEXOCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `nexocode`، أو عيّن `NEXOCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `nexocode serve` و `nexocode web`.
...
عند تشغيل `nexocode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم nexocode للتفاعل مع nexocode برمجيا.
:::

تتيح هذه البنية لـ nexocode دعم عدة عملاء وتمكّنك من التفاعل مع nexocode برمجيا.

يمكنك تشغيل `nexocode serve` لبدء خادم مستقل. إذا كانت واجهة nexocode في terminal (TUI)
قيد التشغيل، فسيبدأ `nexocode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات NexoCode لـ [IDE](/docs/ide).
...
يعرض خادم nexocode واجهات API التالية.
