...
تتيح مهارات الوكيل لـ NexoCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث NexoCode في هذه المواقع:

- إعدادات المشروع: `.nexocode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/nexocode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج NexoCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.nexocode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/nexocode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.nexocode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض NexoCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `nexocode.json`:
...
**للوكلاء المدمجين** (في `nexocode.json`):
