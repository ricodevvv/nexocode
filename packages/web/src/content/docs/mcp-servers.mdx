...
You can add external tools to NexoCode using the _Model Context Protocol_, or MCP. NexoCode supports both local and remote servers.
...
You can define MCP servers in your [NexoCode Config](https://nexocode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/nexocode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
NexoCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, NexoCode will:
...
If the server requires authentication, NexoCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `nexocode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, NexoCode will store the tokens securely in `~/.local/share/nexocode/mcp-auth.json`.
...
Your MCPs are available as tools in NexoCode, alongside built-in tools. So you can manage them through the NexoCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect NexoCode to your Sentry account.
