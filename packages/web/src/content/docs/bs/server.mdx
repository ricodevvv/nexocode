---
title: Server
description: Komunicirajte s nexocode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `nexocode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji nexocode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `nexocode.local` |
...
Postavite `NEXOCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `nexocode`, ili postavite `NEXOCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `nexocode serve` i za `nexocode web`.
...
Kada pokrenete `nexocode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite nexocode server za programsku interakciju sa nexocode-om.
:::

Ova arhitektura omogucava nexocode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `nexocode serve` da startate standalone server. Ako je nexocode TUI vec pokrenut, `nexocode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste NexoCode [IDE](/docs/ide) pluginovi.
...
nexocode server izlaže sljedece API-je.
