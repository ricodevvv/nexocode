...
Da biste otklonili probleme s NexoCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `nexocode --log-level DEBUG`.

---

## Pohrana

NexoCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
NexoCode Desktop pokreće lokalni NexoCode server (`nexocode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `NexoCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/nexocode/nexocode.jsonc` (ili `~/.config/nexocode/nexocode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/nexocode/nexocode.jsonc`
...
NexoCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga NexoCode mogao ponovo izgraditi.

1. Potpuno zatvorite NexoCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/nexocode`
- **Linux**: obrišite `~/.cache/nexocode` (ili pokrenite `rm -rf ~/.cache/nexocode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\nexocode`

3. Ponovo pokrenite NexoCode Desktop.

---

### Rješavanje problema sa vezom na serveru

NexoCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `nexocode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `NEXOCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `NEXOCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, NexoCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa NexoCode karakteristikama.

---

### Obavještenja se ne prikazuju

NexoCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za NexoCode u postavkama vašeg OS-a, i
...
1. Zatvorite NexoCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka NexoCode Desktop aplikacije):

- `nexocode.settings.dat` (URL zadanog servera za desktop)
- `nexocode.global.dat` i `nexocode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `nexocode models`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. NexoCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite NexoCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti NexoCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
NexoCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
