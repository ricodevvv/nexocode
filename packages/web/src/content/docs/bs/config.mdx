---
title: Konfiguracija
description: Korištenje NexoCode JSON konfiguracije.
---

Možete konfigurirati NexoCode koristeći JSON konfiguracijski fajl.

---

## Format

NexoCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/nexocode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/nexocode/nexocode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`NEXOCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`nexocode.json` u projektu) - postavke specifične za projekat
5. **`.nexocode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`NEXOCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.nexocode` i `~/.config/nexocode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/nexocode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu NexoCode konfiguraciju u `~/.config/nexocode/nexocode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/nexocode/tui.json`.
...
Dodajte `nexocode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se NexoCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `NEXOCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `NEXOCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.nexocode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.nexocode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`nexocode.ai/config.json`**](https://nexocode.ai/config.json).

TUI konfiguracija koristi [**`nexocode.ai/tui.json`**](https://nexocode.ai/tui.json).
...
Koristite `NEXOCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `nexocode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `nexocode serve` i `nexocode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš NexoCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `nexocode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj NexoCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, NexoCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/nexocode/agents/` ili `.nexocode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, NexoCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`nexocode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/nexocode/commands/` ili `.nexocode/commands/`. [Saznajte više ovdje](/docs/commands).
...
NexoCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, NexoCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju NexoCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.nexocode/plugins/` ili `~/.config/nexocode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite NexoCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
