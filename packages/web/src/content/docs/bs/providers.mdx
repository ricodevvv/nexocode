---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u NexoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

NexoCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj NexoCode konfiguraciji.
...
u `~/.local/share/nexocode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem NexoCode
config.
...
NexoCode Zen je lista modela koje je obezbedio NexoCode tim koji su bili
testirano i potvrđeno da dobro radi sa NexoCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa NexoCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite nexocode i idite na [nexocode.ai/auth](https://nexocode.ai/auth).
...
Radi kao i svaki drugi provajder u NexoCode i potpuno je opcionalan za korištenje.

---

## NexoCode Go

NexoCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža NexoCode tim i koji su testirani i verificirani da dobro rade s NexoCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `NexoCode Go` i idite na [nexocode.ai/auth](https://nexocode.ai/zen).
...
Da biste koristili Amazon Bedrock s NexoCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete nexocode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `nexocode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u NexoCode.
:::
...
Možete konfigurirati nexocode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi nexocode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju nexocode konfiguraciju.
...
da pohrani token u nexocode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
NexoCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate NexoCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`nexocode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `nexocode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s nexocode:
...
Za korištenje Google Vertex AI s NexoCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće nexocode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem nexocodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki NexoCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki NexoCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati nexocode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati nexocode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati nexocode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za NexoCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s NexoCode:
...
3. Kopirajte API ključ za korištenje u NexoCode.
...
6. **Važno**: Prije upotrebe modela oblaka u NexoCode, morate lokalno povući informacije o modelu:
...
NexoCode Zen je lista testiranih i verifikovanih modela koju je obezbedio NexoCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>NexoCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **NexoCode Zen**.

```txt
   /connect
```

3. Unesite svoj NexoCode API ključ.
...
Također možete dodati dodatne modele putem vaše nexocode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše nexocode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Nexocodeom:
...
Također možete prilagoditi modele kroz svoju nexocode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s nexocode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `nexocode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju NexoCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `nexocode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite NexoCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj nexocode konfiguraciji.
