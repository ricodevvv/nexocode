...
U NexoCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, NexoCode koristi nasu `nexocode` temu.
...
NexoCode dolazi sa vise ugradenih tema.
...
- Zele da NexoCode odgovara izgledu njihovog terminala
...
NexoCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/nexocode/themes/*.json` ili `$XDG_CONFIG_HOME/nexocode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.nexocode/themes/*.json`
4. **Trenutni radni direktorij** - `./.nexocode/themes/*.json`
