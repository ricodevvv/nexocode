...
Možete dodati vanjske alate u NexoCode koristeći _Model Context Protocol_, ili MCP. NexoCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [NexoCode Config](https://nexocode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/nexocode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
NexoCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, NexoCode će:
...
Ako server zahtijeva autentifikaciju, NexoCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `nexocode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, NexoCode sigurno čuva tokene u `~/.local/share/nexocode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u NexoCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz NexoCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete NexoCode sa vašim Sentry nalogom.
