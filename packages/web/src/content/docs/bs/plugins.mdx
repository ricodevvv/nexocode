---
title: Dodaci
description: Napišite vlastite dodatke za proširenje NexoCode.
---

Dodaci vam omogućavaju da proširite NexoCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja NexoCode.
...
- `.nexocode/plugins/` - Dodaci na nivou projekta
- `~/.config/nexocode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/nexocode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/nexocode/nexocode.json`)
2. Konfiguracija projekta (`nexocode.json`)
3. Globalni direktorij dodataka (`~/.config/nexocode/plugins/`)
4. Direktorij dodataka projekta (`.nexocode/plugins/`)
...
NexoCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Nexocode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje NexoCode.
...
Spriječite nexocode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u nexocode:
...
Pomoćnik `tool` kreira prilagođeni alat koji nexocode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za nexocode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://nexocode.ai/docs/sdk) za detalje.
