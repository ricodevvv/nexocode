...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. NexoCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u NexoCode.
...
:::note
Ovaj alat je dostupan samo kada je `NEXOCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `NEXOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz NexoCode provajdera ili kada je varijabla `NEXOCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju NexoCode:
