...
Создайте `.nexocode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию nexocode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем nexocode [config](/docs/config):
...
- Глобальный: `~/.config/nexocode/commands/`
- Для каждого проекта: `.nexocode/commands/`
...
nexocode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
