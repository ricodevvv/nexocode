...
description: Skriv dine egne programtillegg for å utvide NexoCode.
---

Plugins lar deg utvide NexoCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til NexoCode.
...
- `.nexocode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/nexocode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/nexocode/nexocode.json`)
2. Prosjektkonfigurasjon (`nexocode.json`)
3. Global plugin-katalog (`~/.config/nexocode/plugins/`)
4. Prosjektpluginkatalog (`.nexocode/plugins/`)
...
NexoCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En NexoCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide NexoCode.
...
:::note
Hvis du bruker NexoCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre NexoCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til NexoCode:
...
`tool`-hjelperen lager et tilpasset verktøy som NexoCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for NexoCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://nexocode.ai/docs/sdk) for detaljer.
