---
title: Tjener
description: Samhandle med NexoCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `nexocode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en NexoCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `nexocode.local` |
...
Sett `NEXOCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `nexocode` som standard, eller sett `NEXOCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `nexocode serve` og `nexocode web`.
...
Når du kjører `nexocode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk NexoCode-serveren til å samhandle med NexoCode programmatisk.
:::

Denne arkitekturen lar NexoCode støtte flere klienter og lar deg samhandle med NexoCode programmatisk.

Du kan kjøre `nexocode serve` for å starte en frittstående server. Hvis du har
NexoCode TUI kjører, vil `nexocode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av NexoCode [IDE](/docs/ide) plugins.
...
nexocode-serveren viser følgende APIer.
