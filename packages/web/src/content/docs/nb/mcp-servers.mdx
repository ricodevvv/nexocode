...
Du kan legge til eksterne verktøy i NexoCode ved å bruke _Model Context Protocol_ eller MCP. NexoCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [NexoCode Config](https://nexocode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="nexocode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/nexocode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
NexoCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil NexoCode:
...
Hvis serveren krever autentisering, vil NexoCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `nexocode mcp auth <server-name>`.
...
```bash
nexocode mcp auth my-oauth-server
```
...
```bash
nexocode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil NexoCode lagre tokens sikkert i `~/.local/share/nexocode/mcp-auth.json`.
...
nexocode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i NexoCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom NexoCode-konfigurasjonen som alle andre verktøy.
...
```json title="nexocode.json" {11, 14-18}
{
  "$schema": "https://nexocode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
nexocode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble NexoCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver NexoCode-samtale oppført som en separat økt.
