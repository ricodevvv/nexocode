...
Create `.nexocode/commands/test.md`:
...
You can add custom commands through the NexoCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your NexoCode [config](/docs/config):
...
nexocode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
