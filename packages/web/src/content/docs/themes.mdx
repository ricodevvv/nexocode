...
With NexoCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, NexoCode uses our own `nexocode` theme.
...
NexoCode comes with several built-in themes.
...
- Want NexoCode to match their terminal's appearance
...
NexoCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/nexocode/themes/*.json` or `$XDG_CONFIG_HOME/nexocode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.nexocode/themes/*.json`
4. **Current working directory** - Defined in `./.nexocode/themes/*.json`
