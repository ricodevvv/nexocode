...
Com o nexocode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o nexocode usa nosso próprio tema `nexocode`.
...
O nexocode vem com vários temas embutidos.
...
- Querem que o nexocode corresponda à aparência do seu terminal
...
O nexocode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/nexocode/themes/*.json` ou `$XDG_CONFIG_HOME/nexocode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.nexocode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.nexocode/themes/*.json`
