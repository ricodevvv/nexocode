---
title: Servidor
description: Interaja com o servidor nexocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `nexocode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente nexocode pode usar.
...
```bash
nexocode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `nexocode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
nexocode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `NEXOCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `nexocode`, ou defina `NEXOCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `nexocode serve` quanto ao `nexocode web`.

```bash
NEXOCODE_SERVER_PASSWORD=your-password nexocode serve
```
...
Quando você executa `nexocode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor nexocode para interagir com o nexocode programaticamente.
:::

Essa arquitetura permite que o nexocode suporte múltiplos clientes e permite que você interaja com o nexocode programaticamente.

Você pode executar `nexocode serve` para iniciar um servidor autônomo. Se você tiver o TUI do nexocode em execução, `nexocode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do nexocode [IDE](/docs/ide).
...
O servidor nexocode expõe as seguintes APIs.
