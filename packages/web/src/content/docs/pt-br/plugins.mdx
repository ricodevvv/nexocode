---
title: Plugins
description: Escreva seus próprios plugins para estender o nexocode.
---

Plugins permitem que você estenda o nexocode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do nexocode.
...
- `.nexocode/plugins/` - Plugins em nível de projeto
- `~/.config/nexocode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/nexocode/node_modules/`.
...
1. Configuração global (`~/.config/nexocode/nexocode.json`)
2. Configuração do projeto (`nexocode.json`)
3. Diretório de plugins global (`~/.config/nexocode/plugins/`)
4. Diretório de plugins do projeto (`.nexocode/plugins/`)
...
O nexocode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do nexocode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o nexocode.
...
:::note
Se você estiver usando o aplicativo desktop nexocode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o nexocode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao nexocode:
...
O helper `tool` cria uma ferramenta personalizada que o nexocode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o nexocode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://nexocode.ai/docs/sdk) para detalhes.
