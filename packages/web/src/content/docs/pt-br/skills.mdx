...
As habilidades do agente permitem que o nexocode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O nexocode pesquisa nesses locais:

- Configuração do projeto: `.nexocode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/nexocode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o nexocode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.nexocode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/nexocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.nexocode/skills/git-release/SKILL.md` assim:
...
O nexocode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `nexocode.json`:
...
**Para agentes embutidos** (em `nexocode.json`):
