...
Agent Skills erlauben NexoCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
NexoCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft NexoCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.nexocode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/nexocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.nexocode/skills/git-release/SKILL.md` zum Beispiel so:
...
NexoCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `nexocode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `nexocode.json`):
