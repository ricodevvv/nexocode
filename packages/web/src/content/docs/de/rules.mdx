---
title: Regeln
description: Lege eigene Anweisungen fuer nexocode fest.
---

Du kannst nexocode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in nexocode den Befehl `/init` aus.
...
So kann nexocode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht nexocode sie zu erweitern.
...
nexocode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/nexocode/AGENTS.md` speichern.
Diese gelten in allen nexocode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt NexoCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/nexocode/AGENTS.md` existiert)
...
Beim Start sucht nexocode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/nexocode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `nexocode.json` oder in `~/.config/nexocode/nexocode.json` angeben.
...
nexocode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit nexocode.json

Empfohlen ist das `instructions`-Feld in der `nexocode.json`:
...
Du kannst nexocode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `nexocode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
