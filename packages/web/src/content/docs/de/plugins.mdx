---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um NexoCode zu erweitern.
---

Mit Plugins können Sie NexoCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von NexoCode zu ändern.
...
- `.nexocode/plugins/` – Plugins auf Projektebene
- `~/.config/nexocode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/nexocode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/nexocode/nexocode.json`)
2. Projektkonfiguration (`nexocode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/nexocode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.nexocode/plugins/`)
...
NexoCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein NexoCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie NexoCode erweitern können.
...
:::note
Wenn Sie die NexoCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass NexoCode `.env`-Dateien liest:
...
Plugins können NexoCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das NexoCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für NexoCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://nexocode.ai/docs/sdk).
