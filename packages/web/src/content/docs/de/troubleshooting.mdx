...
Wenn NexoCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `nexocode --log-level DEBUG`.

---

## Speicher

nexocode speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
NexoCode Desktop startet im Hintergrund einen lokalen NexoCode-Server (`nexocode-cli`-Sidecar).
...
- Nur macOS: `NexoCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/nexocode/nexocode.jsonc` (oder `~/.config/nexocode/nexocode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/nexocode/nexocode.jsonc`
...
NexoCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit NexoCode ihn neu aufbauen kann.

1. Beende NexoCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/nexocode`
- **Linux**: loesche `~/.cache/nexocode` (oder fuehre aus: `rm -rf ~/.cache/nexocode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\nexocode`

3. Starte NexoCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

NexoCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `nexocode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `NEXOCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `NEXOCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt NexoCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit NexoCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

NexoCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer NexoCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende NexoCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von NexoCode Desktop):

- `nexocode.settings.dat` (desktop default server URL)
- `nexocode.global.dat` und `nexocode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit NexoCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `nexocode models` aus.
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. nexocode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
2. Starte nexocode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt nexocode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
nexocode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
