...
Mit NexoCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet NexoCode das `nexocode`-Theme.
...
NexoCode liefert mehrere eingebaute Themes mit.
...
- NexoCode optisch ans Terminal anpassen wollen
...
NexoCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/nexocode/themes/*.json` oder `$XDG_CONFIG_HOME/nexocode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.nexocode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.nexocode/themes/*.json`
