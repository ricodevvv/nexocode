---
title: Servidor
description: Interactuar con el servidor nexocode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `nexocode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente nexocode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `nexocode.local` |
...
Configure `NEXOCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `nexocode`, o configure `NEXOCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `nexocode serve` como a `nexocode web`.
...
Cuando ejecuta `nexocode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor nexocode para interactuar con nexocode mediante programación.
:::

Esta arquitectura permite que nexocode admita múltiples clientes y le permite interactuar con nexocode mediante programación.

Puede ejecutar `nexocode serve` para iniciar un servidor independiente. Si tienes el
nexocode TUI ejecutándose, `nexocode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos NexoCode [IDE](/docs/ide).
...
El servidor nexocode expone las siguientes API.
