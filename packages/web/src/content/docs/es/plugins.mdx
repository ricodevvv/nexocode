---
title: Complementos
description: Escriba sus propios complementos para extender NexoCode.
---

Los complementos le permiten extender NexoCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de NexoCode.
...
- `.nexocode/plugins/` - Complementos a nivel de proyecto
- `~/.config/nexocode/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/nexocode/node_modules/`.
...
1. Configuración global (`~/.config/nexocode/nexocode.json`)
2. Configuración del proyecto (`nexocode.json`)
3. Directorio global de complementos (`~/.config/nexocode/plugins/`)
4. Directorio de complementos del proyecto (`.nexocode/plugins/`)
...
```json title=".nexocode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
NexoCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".nexocode/plugins/my-plugin.ts"
...
```js title=".nexocode/plugins/example.js"
...
- `client`: Un cliente SDK nexocode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar nexocode.
...
```js title=".nexocode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "nexocode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio NexoCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que nexocode lea archivos `.env`:

```javascript title=".nexocode/plugins/env-protection.js"
...
```javascript title=".nexocode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a nexocode:

```ts title=".nexocode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@nexocode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que nexocode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para nexocode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://nexocode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".nexocode/plugins/compaction.ts"
import type { Plugin } from "@nexocode-ai/plugin"
...
```ts title=".nexocode/plugins/custom-compaction.ts"
import type { Plugin } from "@nexocode-ai/plugin"
