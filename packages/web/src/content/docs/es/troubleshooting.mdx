...
Para depurar problemas con NexoCode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/nexocode/log/`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `nexocode --log-level DEBUG`.

---

## Almacenamiento

nexocode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/nexocode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\nexocode`
...
NexoCode Desktop ejecuta un servidor NexoCode local (el sidecar `nexocode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `NexoCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/nexocode/nexocode.jsonc` (o `~/.config/nexocode/nexocode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/nexocode/nexocode.jsonc`
...
NexoCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/nexocode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\nexocode\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.nexocode/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que NexoCode pueda reconstruirlo.

1. Salga de NexoCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/nexocode`
- **Linux**: eliminar `~/.cache/nexocode` (o ejecutar `rm -rf ~/.cache/nexocode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\nexocode`

3. Reinicie el escritorio NexoCode.

---

### Solucionar problemas de conexión del servidor

NexoCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `nexocode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `NEXOCODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `NEXOCODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio NexoCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de NexoCode.

---

### Notificaciones que no se muestran

NexoCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para NexoCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio NexoCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio NexoCode):

- `nexocode.settings.dat` (URL del servidor predeterminado de escritorio)
- `nexocode.global.dat` y `nexocode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con NexoCode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/ricodevvv/nexocode/issues**](https://github.com/ricodevvv/nexocode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**nexocode.ai/discord**](https://nexocode.ai/discord)
...
3. Asegúrese de tener la última versión con `nexocode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `nexocode models`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. nexocode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/nexocode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\nexocode`

2. Reinicie nexocode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a nexocode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
nexocode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
