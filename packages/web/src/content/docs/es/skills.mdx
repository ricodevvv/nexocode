...
Las habilidades del agente permiten a NexoCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
NexoCode busca estas ubicaciones:

- Configuración del proyecto: `.nexocode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/nexocode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, NexoCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.nexocode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/nexocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.nexocode/skills/git-release/SKILL.md` así:
...
NexoCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `nexocode.json`:
...
**Para agentes integrados** (en `nexocode.json`):
