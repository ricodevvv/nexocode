...
Las herramientas permiten que LLM realice acciones en su código base. NexoCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en NexoCode.
...
:::note
Esta herramienta solo está disponible cuando `NEXOCODE_EXPERIMENTAL_LSP_TOOL=true` (o `NEXOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor NexoCode o cuando la variable de entorno `NEXOCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar NexoCode:

```bash
NEXOCODE_ENABLE_EXA=1 nexocode
```
