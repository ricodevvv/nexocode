...
description: Cree herramientas que LLM pueda llamar en nexocode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de nexocode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.nexocode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/nexocode/tools/`.
...
```ts title=".nexocode/tools/database.ts" {1}
...
```ts title=".nexocode/tools/math.ts"
import { tool } from "@nexocode-ai/plugin"
...
```ts title=".nexocode/tools/bash.ts"
import { tool } from "@nexocode-ai/plugin"
...
```ts title=".nexocode/tools/project.ts" {8}
import { tool } from "@nexocode-ai/plugin"
...
```python title=".nexocode/tools/add.py"
import sys
...
```ts title=".nexocode/tools/python-add.ts" {10}
import { tool } from "@nexocode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".nexocode/tools/add.py")
