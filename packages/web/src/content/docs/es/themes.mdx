...
Con NexoCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, NexoCode usa nuestro propio tema `nexocode`.
...
NexoCode viene con varios temas integrados.
...
- Quiere que NexoCode coincida con la apariencia de su terminal
...
NexoCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/nexocode/themes/*.json` o `$XDG_CONFIG_HOME/nexocode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.nexocode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.nexocode/themes/*.json`
