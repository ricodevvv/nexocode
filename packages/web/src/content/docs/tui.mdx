---
title: TUI
description: Using the NexoCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

NexoCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running NexoCode starts the TUI for the current directory.
...
When using the NexoCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to NexoCode. Allows you to select from available providers and add their API keys.
...
Exit NexoCode. _Aliases_: `/quit`, `/q`
...
This is separate from `nexocode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long NexoCode waits after the leader key. Defaults to `2000`.
...
Use `NEXOCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `nexocode.default`.
