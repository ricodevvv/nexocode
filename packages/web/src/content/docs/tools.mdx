...
Tools allow the LLM to perform actions in your codebase. NexoCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in NexoCode.
...
:::note
This tool is only available when `NEXOCODE_EXPERIMENTAL_LSP_TOOL=true` (or `NEXOCODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the NexoCode or NexoCode Go provider, or when either the `NEXOCODE_ENABLE_EXA` or `NEXOCODE_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching NexoCode:

```bash
NEXOCODE_ENABLE_EXA=1 nexocode
# or
NEXOCODE_ENABLE_PARALLEL=1 nexocode
```
