...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez NexoCode.
...
Voici plusieurs modèles qui fonctionnent bien avec NexoCode, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration NexoCode.

```json title="nexocode.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [NexoCode Zen](/docs/zen), vous utiliserez `nexocode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="nexocode.jsonc" {7-12,19-24}
{
  "$schema": "https://nexocode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="nexocode.jsonc" {6-21}
{
  "$schema": "https://nexocode.ai/config.json",
  "provider": {
    "nexocode": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. NexoCode est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

NexoCode est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="nexocode.jsonc" {7-18}
{
  "$schema": "https://nexocode.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque NexoCode démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration NexoCode.
