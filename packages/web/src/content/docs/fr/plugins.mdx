---
title: Plugins
description: Écrivez vos propres plugins pour étendre NexoCode.
---

Les plugins vous permettent d'étendre NexoCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de NexoCode.
...
- `.nexocode/plugins/` - Plugins au niveau du projet
- `~/.config/nexocode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/nexocode/node_modules/`.
...
1. Configuration globale (`~/.config/nexocode/nexocode.json`)
2. Configuration du projet (`nexocode.json`)
3. Répertoire global des plugins (`~/.config/nexocode/plugins/`)
4. Répertoire des plugins du projet (`.nexocode/plugins/`)
...
NexoCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK nexocode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre nexocode.
...
:::note
Si vous utilisez l'application de bureau NexoCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez nexocode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à nexocode :
...
L'assistant `tool` crée un outil personnalisé que nexocode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour nexocode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://nexocode.ai/docs/sdk) pour plus de détails.
