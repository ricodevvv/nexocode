...
Avec NexoCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, NexoCode utilise notre propre thème `nexocode`.
...
NexoCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que NexoCode corresponde à l'apparence de leur terminal
...
NexoCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/nexocode/themes/*.json` ou `$XDG_CONFIG_HOME/nexocode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.nexocode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.nexocode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/nexocode/themes
vim ~/.config/nexocode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .nexocode/themes
vim .nexocode/themes/my-theme.json
```
...
{
  "$schema": "https://nexocode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
