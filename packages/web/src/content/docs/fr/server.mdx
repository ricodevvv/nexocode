---
title: Serveur
description: Interagissez avec le serveur nexocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `nexocode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client nexocode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `nexocode.local` |
...
Définissez `NEXOCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `nexocode`, ou définissez `NEXOCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `nexocode serve` et à `nexocode web`.
...
Lorsque vous exécutez `nexocode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur nexocode pour interagir avec nexocode par programmation.
:::

Cette architecture permet à nexocode de prendre en charge plusieurs clients et vous permet d'interagir avec nexocode par programme.

Vous pouvez exécuter `nexocode serve` pour démarrer un serveur autonome. Si vous avez le
nexocode TUI en cours d'exécution, `nexocode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins NexoCode [IDE](/docs/ide).
...
Le serveur nexocode expose les API suivantes.
