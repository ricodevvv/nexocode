---
title: Server
description: Interager med nexocode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `nexocode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en nexocode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `nexocode.local` |
...
Indstil `NEXOCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `nexocode`, eller indstil `NEXOCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `nexocode serve` og `nexocode web`.
...
Når du kører `nexocode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug nexocode-serveren til at interagere med nexocode programmatisk.
:::

Denne arkitektur lader nexocode understøtte flere klienter og lader dig interagere med nexocode programmatisk.

Du kan køre `nexocode serve` for at starte en selvstændig server. Hvis du har
nexocode TUI kørende, vil `nexocode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af NexoCode [IDE](/docs/ide) plugins.
...
NexoCode-serveren viser følgende API'er.
