...
Agentferdigheter lar NexoCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
NexoCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.nexocode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/nexocode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går NexoCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.nexocode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/nexocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.nexocode/skills/git-release/SKILL.md` slik:
...
NexoCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `nexocode.json`:
...
**For innebygde agenter** (i `nexocode.json`):
