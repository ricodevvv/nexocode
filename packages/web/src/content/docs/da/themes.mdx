...
Med NexoCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger NexoCode vårt eget `nexocode`-tema.
...
NexoCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/nexocode/themes/*.json` eller `$XDG_CONFIG_HOME/nexocode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.nexocode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.nexocode/themes/*.json`
