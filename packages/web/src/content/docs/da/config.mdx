---
title: Konfiguration
description: Ved at bruge NexoCode JSON konfig.
---

Du kan konfigurere NexoCode ved hjælp af en JSON-konfigurationsfil.

---

## Format

NexoCode understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/nexocode`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/nexocode/nexocode.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`NEXOCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`nexocode.json` i projekt) - projektspecifikke indstillinger
5. **`.nexocode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`NEXOCODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.nexocode` og `~/.config/nexocode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/nexocode`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale NexoCode-konfiguration i `~/.config/nexocode/nexocode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `nexocode.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når NexoCode starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `NEXOCODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `NEXOCODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.nexocode` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.nexocode` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`nexocode.ai/config.json`**](https://nexocode.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `nexocode serve` og `nexocode web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din NexoCode-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `nexocode.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din NexoCode-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger NexoCode at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din NexoCode-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/nexocode/agents/` eller `.nexocode/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil NexoCode falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`nexocode run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/nexocode/commands/` eller `.nexocode/commands/`. [Learn more here](/docs/commands).
...
NexoCode vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard nexocode **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide NexoCode med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.nexocode/plugins/` eller `~/.config/nexocode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse NexoCode til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
