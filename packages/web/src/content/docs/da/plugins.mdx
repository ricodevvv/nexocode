---
title: Plugins
description: Skriv dine egne plugins for at udvide NexoCode.
---

Plugins giver dig mulighed for at udvide NexoCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre NexoCodes standardadfærd.
...
- `.nexocode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/nexocode/node_modules/`.
...
1. Global konfiguration (`~/.config/nexocode/nexocode.json`)
2. Projektkonfiguration (`nexocode.json`)
3. Global plugin-mappe (`~/.config/nexocode/plugins/`)
4. Projekt plugin bibliotek (`.nexocode/plugins/`)
...
NexoCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En nexocode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide nexocode.
...
:::note
Hvis du bruger NexoCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå nexocode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til nexocode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som nexocode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for nexocode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://nexocode.ai/docs/sdk) for detaljer.
