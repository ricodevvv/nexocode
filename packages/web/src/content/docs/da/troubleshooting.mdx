...
For at fejlfinde problemer med NexoCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `nexocode --log-level DEBUG`.

---

## Lagring

nexocode gemmer sessionsdata og andre applikationsdata på disken på:
...
NexoCode Desktop kører en lokal NexoCode-server (`nexocode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `NexoCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/nexocode/nexocode.jsonc` (eller `~/.config/nexocode/nexocode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/nexocode/nexocode.jsonc`
...
NexoCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så NexoCode kan genopbygge den.

1. Afslut NexoCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/nexocode`
- **Linux**: slet `~/.cache/nexocode` (eller kør `rm -rf ~/.cache/nexocode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\nexocode`

3. Start NexoCode Desktop på ny.

---

### Løs problemer med serverforbindelse

NexoCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `nexocode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `NEXOCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `NEXOCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver NexoCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i NexoCode.

---

### Meddelelser vises ikke

NexoCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for NexoCode i dine OS-indstillinger, og
...
1. Afslut NexoCode Desktop.
2. Find og slet disse filer (de findes i NexoCode Desktop-appens datamappe):

- `nexocode.settings.dat` (skrivebordsstandardserver URL)
- `nexocode.global.dat` og `nexocode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med NexoCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `nexocode models`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. nexocode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start nexocode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge nexocode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
nexocode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
