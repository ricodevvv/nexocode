---
title: Server
description: Interact with nexocode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `nexocode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an nexocode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `nexocode.local` |
...
Set `NEXOCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `nexocode`, or set `NEXOCODE_SERVER_USERNAME` to override it. This applies to both `nexocode serve` and `nexocode web`.
...
When you run `nexocode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the nexocode server to interact with nexocode programmatically.
:::

This architecture lets nexocode support multiple clients and allows you to interact with nexocode programmatically.

You can run `nexocode serve` to start a standalone server. If you have the
nexocode TUI running, `nexocode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the NexoCode [IDE](/docs/ide) plugins.
...
The nexocode server exposes the following APIs.
