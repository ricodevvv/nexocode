---
title: Config
description: Using the NexoCode JSON config.
---

You can configure NexoCode using a JSON config file.

---

## Format

NexoCode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="nexocode.jsonc"
{
...
1. **Remote config** (from `.well-known/nexocode`) - organizational defaults
2. **Global config** (`~/.config/nexocode/nexocode.json`) - user preferences
3. **Custom config** (`NEXOCODE_CONFIG` env var) - custom overrides
4. **Project config** (`nexocode.json` in project) - project-specific settings
5. **`.nexocode` directories** - agents, commands, plugins
6. **Inline config** (`NEXOCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/nexocode/` on macOS) - admin-controlled
...
:::note
The `.nexocode` and `~/.config/nexocode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/nexocode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/nexocode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global NexoCode config in `~/.config/nexocode/nexocode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/nexocode/tui.json`.
...
Add `nexocode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When NexoCode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `NEXOCODE_CONFIG` environment variable.

```bash
export NEXOCODE_CONFIG=/path/to/my/custom-config.json
nexocode run "Hello world"
```
...
Specify a custom config directory using the `NEXOCODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.nexocode` directory, and should
follow the same structure.

```bash
export NEXOCODE_CONFIG_DIR=/path/to/my/config-directory
nexocode run "Hello world"
```

The custom directory is loaded after the global config and `.nexocode` directories, so it **can override** their settings.
...
Drop an `nexocode.json` or `nexocode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/nexocode/` |
| Linux    | `/etc/nexocode/`                         |
| Windows  | `%ProgramData%\nexocode`                 |
...
On macOS, NexoCode reads managed preferences from the `ai.nexocode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

NexoCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.nexocode.managed.plist`
2. `/Library/Managed Preferences/ai.nexocode.managed.plist`

The plist keys map directly to `nexocode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.nexocode.managed` PayloadType. The NexoCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.nexocode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.nexocode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.nexocode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`nexocode.ai/config.json`**](https://nexocode.ai/config.json).

TUI config uses [**`nexocode.ai/tui.json`**](https://nexocode.ai/tui.json).
...
Use `NEXOCODE_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `nexocode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `nexocode serve` and `nexocode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your NexoCode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `nexocode.local`. Useful for running multiple instances on the same network.
...
If not specified, NexoCode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your NexoCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, NexoCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny NexoCode actions on configured resources. Currently, policies can control which providers NexoCode may use.
...
NexoCode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, NexoCode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/nexocode/agents/` or `.nexocode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, NexoCode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`nexocode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/nexocode/commands/` or `.nexocode/commands/`. [Learn more here](/docs/commands).
...
NexoCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
NexoCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, nexocode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend NexoCode with custom tools, hooks, and integrations.

Place plugin files in `.nexocode/plugins/` or `~/.config/nexocode/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict NexoCode to only use specific providers rather than disabling them one by one.
