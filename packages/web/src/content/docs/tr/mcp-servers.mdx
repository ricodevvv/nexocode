...
_Model Bağlam Protokolü_ veya MCP kullanarak nexocode'a harici araçlar ekleyebilirsiniz. nexocode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [nexocode Config](https://nexocode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/nexocode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
nexocode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde nexocode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa nexocode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `nexocode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından nexocode, belirteçleri `~/.local/share/nexocode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra nexocode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi nexocode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve nexocode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
