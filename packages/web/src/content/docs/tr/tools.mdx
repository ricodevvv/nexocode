...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. nexocode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
nexocode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `NEXOCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `NEXOCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca nexocode provider kullanılırken veya `NEXOCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

nexocode başlatırken etkinleştirmek için:
