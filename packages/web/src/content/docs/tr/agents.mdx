...
nexocode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
nexocode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
NexoCode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

NexoCode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `nexocode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse nexocode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global nexocode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
nexocode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [NexoCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `nexocode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `nexocode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/ricodevvv/nexocode).
:::
