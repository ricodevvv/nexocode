...
nexocode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\nexocode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `nexocode --log-level DEBUG`.

---

## Depolama

nexocode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:
...
nexocode Desktop arka planda yerel bir nexocode sunucusu (`nexocode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `nexocode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/nexocode/nexocode.jsonc` (veya `~/.config/nexocode/nexocode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/nexocode/nexocode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\nexocode\nexocode.jsonc` yapistirin
...
nexocode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/nexocode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\nexocode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), nexocode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. nexocode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/nexocode`
- **Linux**: `~/.cache/nexocode` dizinini silin (veya `rm -rf ~/.cache/nexocode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\nexocode` yapistirin

3. nexocode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

nexocode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`nexocode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `NEXOCODE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `NEXOCODE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta nexocode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, nexocode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

nexocode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda nexocode icin bildirimler etkinse
...
1. nexocode Desktop'u kapatin
2. Su dosyalari bulun ve silin (nexocode Desktop uygulama veri dizininde yer alirlar):

- `nexocode.settings.dat` (desktop varsayilan sunucu URL'si)
- `nexocode.global.dat` ve `nexocode.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
nexocode ile ilgili bir sorun yasiyorsaniz:
...
3. `nexocode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `nexocode models` calistirin.
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. nexocode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
2. En guncel provider paketlerini yeniden kurmak icin nexocode'u yeniden baslatin

Bu, nexocode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
nexocode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
