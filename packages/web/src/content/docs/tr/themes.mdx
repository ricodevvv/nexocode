...
nexocode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak nexocode kendi `nexocode` temasını kullanır.
...
nexocode birden fazla yerleşik temayla gelir.
...
- nexocode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
nexocode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/nexocode/themes/*.json` veya `$XDG_CONFIG_HOME/nexocode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.nexocode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.nexocode/themes/*.json`
