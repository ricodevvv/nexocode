---
title: Eklentiler
description: NexoCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek NexoCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya NexoCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.nexocode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/nexocode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/nexocode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/nexocode/nexocode.json`)
2. Project config (`nexocode.json`)
3. Global eklenti dizini (`~/.config/nexocode/plugins/`)
4. Proje eklenti dizini (`.nexocode/plugins/`)
...
NexoCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir NexoCode SDK istemcisi.
...
NexoCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
NexoCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
NexoCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca NexoCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, NexoCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra NexoCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://nexocode.ai/docs/sdk) bakın.
