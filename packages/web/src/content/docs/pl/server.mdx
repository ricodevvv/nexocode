---
title: Serwer
description: Interakcja z serwerem nexocode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `nexocode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient nexocode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `nexocode.local` |
...
Ustaw `NEXOCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `nexocode` lub ustaw `NEXOCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `nexocode serve`, jak i `nexocode web`.
...
Kiedy uruchomisz `nexocode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera nexocode do programowej interakcji z nexocode.
:::

Ta architektura umożliwia obsługę wielu klientów przez nexocode i programową interakcję z kodem otwartym.

Możesz uruchomić `nexocode serve`, aby uruchomić samodzielny serwer. Jeśli masz
nexocode TUI działa, `nexocode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki nexocode [IDE](/docs/ide).
...
Serwer nexocode udostępnia następujące interfejsy API.
