---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć nexocode.
---

Wtyczki umożliwiające wyłączenie nexocode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania nexocode.
...
- `.nexocode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/nexocode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/nexocode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/nexocode/nexocode.json`)
2. Konfiguracja projektu (`nexocode.json`)
3. Globalny katalog wtyczek (`~/.config/nexocode/plugins/`)
4. Katalog wtyczek projektu (`.nexocode/plugins/`)
...
nexocode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z nexocode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do nexocode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej nexocode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj nexocode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do nexocode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod nexocode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://nexocode.ai/docs/sdk).
