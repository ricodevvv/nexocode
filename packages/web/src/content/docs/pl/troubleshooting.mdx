...
Aby debugować problemy z nexocode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `nexocode --log-level DEBUG`.

---

## Przechowywanie danych

nexocode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
nexocode Desktop uruchamia w tle lokalny serwer nexocode (sidecar `nexocode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `NexoCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/nexocode/nexocode.jsonc` (lub `~/.config/nexocode/nexocode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/nexocode/nexocode.jsonc`
...
nexocode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby nexocode mógł ją odbudować.

1. Całkowicie zamknij nexocode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nexocode`
- **Linux**: usuń `~/.cache/nexocode` (lub uruchom `rm -rf ~/.cache/nexocode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\nexocode`

3. Uruchom ponownie nexocode Desktop.

---

### Napraw problemy z połączeniem z serwerem

nexocode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `nexocode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `NEXOCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `NEXOCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows nexocode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami nexocode.

---

### Brak powiadomień

nexocode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla nexocode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie nexocode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji nexocode Desktop):

- `nexocode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `nexocode.global.dat` i `nexocode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z nexocode:
...
3. Upewnij się, że masz najnowszą wersję z `nexocode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `nexocode models`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. nexocode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
2. Uruchom ponownie kod nexocode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to nexocode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
nexocode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
