...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. NexoCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w NexoCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `NEXOCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `NEXOCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy NexoCode lub gdy zmienna środowiskowa `NEXOCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu NexoCode:
