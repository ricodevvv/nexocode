...
Umiejętności agenta pozwalają nexocode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
nexocode przeszukuje te lokalizacje:

- Project config: `.nexocode/skills/<name>/SKILL.md`
- Global config: `~/.config/nexocode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu nexocode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.nexocode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/nexocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.nexocode/skills/git-release/SKILL.md` w ten sposób:
...
nexocode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `nexocode.json`:
...
**Dla agentów wbudowanych** (w `nexocode.json`):
