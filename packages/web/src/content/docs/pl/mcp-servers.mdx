...
Możesz dodać dodatkowe informacje do nexocode za pomocą _Model Context Protocol_, w skrócie MCP. nexocode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [nexocode Config](https://nexocode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/nexocode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
nexocode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, nexocode:
...
Jeśli serwer wymaga uwierzytelnienia, nexocode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `nexocode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji nexocode bezpieczne przechowa tokeny w `~/.local/share/nexocode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w nexocode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie nexocode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć nexocode z kontem Sentry.
