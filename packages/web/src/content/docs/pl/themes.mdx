...
Dzięki nexocode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie nexocode używa naszego własnego motywu `nexocode`.
...
nexocode ma kilka wbudowanych motywów.
...
- Chcą, aby NexoCode pasował do wyglądu ich terminala
...
nexocode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/nexocode/themes/*.json` lub `$XDG_CONFIG_HOME/nexocode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.nexocode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.nexocode/themes/*.json`
